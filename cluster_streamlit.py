...
def cluster_features():
    feat_options = st.multiselect('Top features according to RFE and BORUTA', feats,
                                    default=["Humidity_percent", "Wind_kmph"], max_selections=3)
    ...
    options = ["All"]
    options.extend(range(0, cluster_amount))
    cluster_num = st.radio("Select the cluster to focus: ", options, horizontal=True)

    if cluster_num == "All":
        feature_cluster = px.scatter(data_frame=df, x=feat_options[0], y=feat_options[1], color="cluster")

    elif len(feat_options) == 3:
...
def cluster_local():
    ...
    # Display the feature clusters
    options = ["All"]
    options.extend(range(0, 3))
    cluster_num = st.radio("Select the cluster to focus: ", options, horizontal=True)

    if cluster_num == "All":
        show_df = df
    else:
        show_df = df[df["cluster"] == cluster_num]
